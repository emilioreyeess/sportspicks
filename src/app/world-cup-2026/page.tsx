...
/* ════════════════════════════════════════════════════════════════════════════
   Pre-tournament fixtures (static preview)
   ════════════════════════════════════════════════════════════════════════════ */
...
function PreTournamentFixtures({
  ...
  return (
    <div className="space-y-3">
      ...
      {displayed.length === 0 && (
        <Card variant="flat" className="px-6 py-10">
          <EmptyState
            icon="trophy"
            title={`Sin partidos previstos para el Grupo ${filterGroup}`}
            hint="Los fixtures del grupo se confirmarán el 11 jun 2026."
          />
        </Card>
      )}
      ...
      {displayed.map((fix) => {
        ...
        return (
          <Card key={fix.matchId} variant="default" className="overflow-hidden">
            <div className="px-5 sm:px-6 py-4">
              <div className="flex items-center justify-between mb-3.5">
                <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                  Grupo {fix.group} · {fix.city}
                </span>
                <span className="text-[11px] font-medium text-zinc-500">{dateStr} · {timeStr}</span>
              </div>

              <div className="flex items-center justify-between gap-3">
                <TeamSide team={homeTeam} fallback={fix.homeCode} side="home" />
                <p className="text-[13px] font-semibold text-zinc-600 px-2 shrink-0">vs</p>
                <TeamSide team={awayTeam} fallback={fix.awayCode} side="away" />
              </div>
