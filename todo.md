...
- [x] **B3.** Reemplazar `<img src=match.home_logo>` y `<img src=match.away_logo>` en `src/components/matches/TodayMatches.tsx:MatchCard` por `<TeamCrest>`. — **HECHO**: añadido inferidor `inferIsInternationalFromESPN(competition)` en `crest.ts` (substring match sobre nombre ESPN: friendly/world/nations/copa america/euro/concacaf… + exclusión "club" para Mundial de Clubes). `MatchCard` deriva `isIntl` de `match.league_name` y lo pasa a `<TeamCrest size="sm">`. `tsc` 0, build limpio, `GET /` 200 sin errores de hidratación.
- [x] **B4.** En `src/app/historico/page.tsx:PickRow`, añadir avatar con `<TeamCrest>` al lado del partido. — **HECHO**: par de crests (home+away) solapados `-space-x-1.5` con `ring-2 ring-zinc-950`, `size="sm"`, `shrink-0` (nunca se comprimen). `isInternational` derivado con `inferIsInternationalFromESPN(pick.league)` (misma función que B3). Texto conserva `flex-1 min-w-0 truncate` → mobile-safe. `tsc` 0, build limpio, `GET /historico` 200 sin hydration errors.
