...
- [ ] **A2.** `src/app/api/bets/auto-extract/route.ts`: reemplazar `stake: cleanBet.totalStake ?? 0` por `stake: cleanBet.totalStake` (que ya viene `null` si la extracción no detectó). Recalcular `potential_return` como `stake != null ? stake * combined_odds : null`. Añadir test inline: si Vision devuelve `totalStake: null`, el row debe quedar con `stake IS NULL` y `needs_review = true`.
- [ ] **A3.** `src/app/api/bets/route.ts` (POST manual): añadir validación de coherencia — si `body.stake == null` o `0`, el bet entra con `needs_review = true` también (consistencia entre vía manual y vía OCR).
- [ ] **A4.** Actualizar `prd.md` sección "Estados de una apuesta" con la regla R1 documentada como invariante.
